...
def sumoDataCollection():
    ...
    if "f_0.0" in vehicles_list:
        try:
            ...
            rounded_position = (round(x_veh1, 2), round(y_veh1, 2), round(z_veh1, 2))
                ...
                for obj_id in context_subscription.keys():
                    if obj_id != "f_0.0" and obj_id in vehicles_list:
                        try:
                            ...
                            rounded_position = (round(x, 3), round(y, 3), round(z, 3))
...
try:
    ...
    while (
                ...
                for vehicle in unity_data:
                    if vehicle["vehicle_id"] == "f_0.0":
                        ...
                        traci.vehicle.moveToXY(
                            vehicle["vehicle_id"], "", 0, x, y, angle, keepRoute=2
                        )
        ...
        if (
            COMPUTE_RTF
            and not rtf_measurement_started
            and current_sim_time >= ExperimentStartTime
        ):
        ...
        if (
            COMPUTE_RTF
            and rtf_measurement_started
            and current_sim_time >= ExperimentStartTime
            and current_sim_time < ExperimentEndTime
        ):
        ...
        vehicle_data_json = json.dumps(
            {"type": "vehicles", "vehicles": vehicle_data}, separators=(",", ":")
        )
        ...
        if sim_t - last_tl_time >= TL_UPDATE_INTERVAL:
            ...
            pub_socket.send_string(
                json.dumps(
                    {"type": "trafficlights", "lights": tl_list}, separators=(",", ":")
                )
            )
            last_tl_time = sim_t

        number_of_cars_in_context = len(
            [v for v in vehicle_data if v["vehicle_id"] != "f_0.0"]
        )
        ...
        context_cars = [
            v["vehicle_id"] for v in vehicle_data if v["vehicle_id"] != "f_0.0"
        ]
                    ...
                    if dt_real > 0:
                        ...
                        if vid != "f_1.0":
                            ...
                            if len(real_world_speed_history[vid]) > WINDOW_SIZE:
                                smoothed_v_real = statistics.mean(
                                    real_world_speed_history[vid][-WINDOW_SIZE:]
                                )
                            else:
                                smoothed_v_real = statistics.mean(
                                    real_world_speed_history[vid]
                            ...
                            if (
                                COMPUTE_CONTEXT_CAR_REAL_WORLD_SPEEDS
                                and current_sim_time >= ExperimentStartTime
                                and vid in context_cars
                            ):
            ...
            if len(data_points) > 1:
                ...
                if v_real is not None:
                    ...
                    if len(real_world_speed_values) > WINDOW_SIZE:
                        smoothed_v_real = statistics.mean(
                            real_world_speed_values[-WINDOW_SIZE:]
                        )
                    else:
    ...
    if COMPUTE_F1_SPEEDS and overall_real_world_speeds:
        ...
        filtered_real_world_speeds = [
            rw_speed
            for dp, rw_speed in zip(data_points, overall_real_world_speeds)
            if dp["simulation_time"] >= ExperimentStartTime
        ]

        if filtered_simulation_speeds:
            overall_avg_simulation_speed = statistics.mean(filtered_simulation_speeds)
            overall_std_simulation_speed = (
                statistics.stdev(filtered_simulation_speeds)
                if len(filtered_simulation_speeds) > 1
                else 0
            )

            logger.info("\n--- Final Statistics (Excluding First 900 Seconds) ---")
            logger.info(
                f"Overall Average Simulation Speed of 'f_1.0': {overall_avg_simulation_speed:.2f} m/s"
            )
            logger.info(
                f"Overall STD Simulation Speed of 'f_1.0': {overall_std_simulation_speed:.2f} m/s"
            )
        else:
            logger.warning(
                "No simulation speed data points after the cutoff time for 'f_1.0'."
            )

        if filtered_real_world_speeds:
            overall_avg_real_world_speed = statistics.mean(filtered_real_world_speeds)
            overall_std_real_world_speed = (
                statistics.stdev(filtered_real_world_speeds)
                if len(filtered_real_world_speeds) > 1
                else 0
            )
            logger.info(
                f"Overall Average Real-World Speed of 'f_1.0': {overall_avg_real_world_speed:.2f} m/s"
            )
            logger.info(
                f"Overall STD Real-World Speed of 'f_1.0': {overall_std_real_world_speed:.2f} m/s"
            )
        else:
            logger.warning(
                "No real-world speed data points after the cutoff time for 'f_1.0'."
            )
    ...
    if COMPUTE_CONTEXT_CAR_SIM_SPEEDS:
        if all_simulation_speeds_list:
            ...
            std_all_sim_speeds = (
                statistics.stdev(all_simulation_speeds_list)
                if len(all_simulation_speeds_list) > 1
                else 0
            )
            logger.info(
                f"Overall Average Simulation Speed of Context Cars: {avg_all_sim_speeds:.2f} m/s"
            )
            logger.info(
                f"Overall STD Simulation Speed of Context Cars: {std_all_sim_speeds:.2f} m/s"
            )
        else:
    ...
    if COMPUTE_CONTEXT_CAR_REAL_WORLD_SPEEDS:
        if all_real_world_speeds_list:
            ...
            std_all_real_speeds = (
                statistics.stdev(all_real_world_speeds_list)
                if len(all_real_world_speeds_list) > 1
                else 0
            )
            logger.info(
                f"Overall Average Real-World Speed of Context Cars: {avg_all_real_speeds:.2f} m/s"
            )
            logger.info(
                f"Overall STD Real-World Speed of Context Cars: {std_all_real_speeds:.2f} m/s"
            )
        else:
    ...
    if COMPUTE_ALL_CARS:
        if cumulative_car_counts:
            ...
            overall_std_car_count = (
                statistics.stdev(cumulative_car_counts)
                if len(cumulative_car_counts) > 1
                else 0
            )
    ...
    if COMPUTE_PROFILING_SUMMARY and num_loops > 0:
        any_profiling_info = (
            COMPUTE_PROFILING_TIMES
            or COMPUTE_CARS_IN_CONTEXT_STATS
            or COMPUTE_MSGS_BYTES_STATS
        )
            ...
            if COMPUTE_PROFILING_TIMES:
                for key in profiling_times:
                    if profiling_times[key]:
                        ...
                        std = (
                            statistics.stdev(profiling_times[key])
                            if len(profiling_times[key]) > 1
                            else 0
                        )
                        logger.info(
                            f"Average {key}: {avg:.6f} seconds (STD: {std:.6f})"
                        )

            total_simulation_time = traci.simulation.getTime()
            avg_sent_messages_per_second = (
                total_sent_messages_count / total_simulation_time
                if total_simulation_time > 0
                else 0
            )
            avg_sent_bytes_per_second = (
                total_sent_bytes_count / total_simulation_time
                if total_simulation_time > 0
                else 0
            )
            avg_received_messages_per_second = (
                total_received_messages_count / total_simulation_time
                if total_simulation_time > 0
                else 0
            )
            avg_received_bytes_per_second = (
                total_received_bytes_count / total_simulation_time
                if total_simulation_time > 0
                else 0
            )

            if COMPUTE_CARS_IN_CONTEXT_STATS and cumulative_context_car_counts:
                overall_avg_context_car_count = statistics.mean(
                    cumulative_context_car_counts
                )
                overall_std_context_car_count = (
                    statistics.stdev(cumulative_context_car_counts)
                    if len(cumulative_context_car_counts) > 1
                    else 0
                )
                logger.info(
                    f"Average Number of Cars in Context: {overall_avg_context_car_count:.2f}"
                )
                logger.info(
                    f"STD Number of Cars in Context: {overall_std_context_car_count:.2f}"
                )

            if COMPUTE_MSGS_BYTES_STATS:
                logger.info(
                    f"Average Messages Sent to Unity: {avg_sent_messages_per_second:.2f} messages/s"
                )
                logger.info(
                    f"Average Bytes Sent to Unity: {avg_sent_bytes_per_second:.2f} B/s"
                )
                logger.info(
                    f"Average Messages Received from Unity: {avg_received_messages_per_second:.2f} messages/s"
                )
                logger.info(
                    f"Average Bytes Received from Unity: {avg_received_bytes_per_second:.2f} B/s"
                )
    ...
    if COMPUTE_RTF:
        ...
        if start_simulation_time is not None and start_wall_time is not None:
            ...
            if total_real_duration > 0:
                ...
                logger.info(
                    f"Real-Time Factor (RTF) from {ExperimentStartTime}s to end of run: {rtf:.2f}"
                )
            else:
                logger.warning(
                    "Real-world duration is zero or negative, cannot compute RTF."
                )
        else:
