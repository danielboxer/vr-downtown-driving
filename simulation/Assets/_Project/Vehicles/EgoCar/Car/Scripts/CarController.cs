...
namespace UnityStandardAssets.Vehicles.Car
{
    ...
    public class CarController : MonoBehaviour
    {
        ...
        public void Move(float steering, float accel, float footbrake, float handbrake)
        {
            ...
            // **ADDED LOGIC HERE**
            // If no handbrake and we start accelerating, release any brake torque on the wheels
            if (handbrake == 0f && accel > 0f)
            {
                for (int i = 0; i < 4; i++)
                {
                    // If the wheel was braked, release it
                    // We assume all wheels can brake (if you want to limit this, you can check conditions)
